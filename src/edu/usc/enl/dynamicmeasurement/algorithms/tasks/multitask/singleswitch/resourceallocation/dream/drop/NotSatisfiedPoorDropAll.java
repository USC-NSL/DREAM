 ...
 * Time: 7:53 PM
 */
public class NotSatisfiedPoorDropAll extends NotSatisfiedPoor {
    private AccuracyAggregator averageToDropAgg;

    public NotSatisfiedPoorDropAll(ThresholdGuaranteeAlgorithm2 algorithm1, PriorityDropPolicy dropPolicy1) {
...
//        averageToDropAgg.update(sumGet - sumGive);
        ...
        if (dropped != null) {
            ...
            while (averageToDrop > 0) {
                ...
                if (dropped1.wasPoor() || dropped1.getTask().getResourceShare() > algorithm.getMinResource()) {
                    output.add(dropped1);
