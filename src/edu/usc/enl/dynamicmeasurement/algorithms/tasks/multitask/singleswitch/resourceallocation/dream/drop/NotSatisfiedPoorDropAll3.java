 ...
 * Time: 7:53 PM
 */
public class NotSatisfiedPoorDropAll3 extends NotSatisfiedPoor {
    private AccuracyAggregator averageToDropAgg;

    public NotSatisfiedPoorDropAll3(ThresholdGuaranteeAlgorithm2 algorithm1, PriorityDropPolicy dropPolicy1) {
...
//        averageToDropAgg.update(sumGet - sumGive);
        TaskRecord2 dropped = dropPolicy.checkForDrop(sumGet > sumGive, tasks);
        if (dropped != null) {
            double averageToDrop = sumGet - sumGive;
            ...
            while (averageToDrop > 0) {
                ...
                if (dropped1.wasPoor()) {
                    ...
                    averageToDrop -= dropped1.getTask().getResourceShare() - minResource;
                } else if (dropped1.wasRich()) {
