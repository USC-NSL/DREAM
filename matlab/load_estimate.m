function [sat,rej,drop]=load_estimate(path1)
    %index=33:(256-32);
    index=69:191;
    ...
    sat=zeros(length(filenames),3);
    rej=zeros(length(filenames),1);
    drop=zeros(length(filenames),1);
    dropage=zeros(length(filenames),1);
