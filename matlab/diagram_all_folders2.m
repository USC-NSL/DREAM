...
for i=1:k-1
    ...
    for j=1:task_num
        ...
        x(1,:)=[];%ignore first epoch as it had no chance
        ...
        if (lifetime==0)
            ...
            if (droped_epochs==0)
                satisfaction(i)=satisfaction(i)+sum(x(x(:,drop_col)==0,precision_col)>=accbound)/lifetime;
            end
        end        
    end
    if (droped_task(i)>0)
    ...
    if (rejected_task(i)<task_num_2)
         satisfaction(i)=satisfaction(i)/(task_num_2-rejected_task(i)-droped_task(i));   
    end
