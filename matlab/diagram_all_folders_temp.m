...
for i=1:k-1
    ...
    x=csvread(sprintf('%s/%s/share.csv',path,filenames{i}),1,0);
    ...
    for j=1:task_num
        ...
        if (lifetime>0)
            s=sum(x(x(:,drop_col)==0,precision_col)>=0.8)/lifetime;
            satisfaction(i)=satisfaction(i)+s;
            temp(j)=s;
        end
    end
    if (droped_task(i)>0)
    ...
    if (rejected_task(i)<task_num_2)
         satisfaction(i)=satisfaction(i)/(task_num_2-rejected_task(i));   
    end 
